...
struct CmosRtcData {
	...
	constexpr bool operator==(const CmosRtcData& other) const = default;
};

static u8 USE_24_HOUR_FORMAT = false;
static u8 USE_BINARY = true;

static DateTime x86_read_rtc() {
	auto read_rtc = []() {
...
struct CmosRtcDateTimeProvider : public DateTimeProvider {
	NTSTATUS get_date_time(DateTime& res) override {
		res = x86_read_rtc();
		return 0;
	}
};
...
void rtc_init() {
	...
	DATE_TIME_PROVIDER = &CMOS_RTC_DATE_TIME_PROVIDER;
}
