...
namespace kstd {
	...
	struct small_function<R(Args...)> {
		...
		requires (!hz::is_same_v<hz::remove_reference_t<F>, small_function> && requires(F f, Args... args) {
			f(std::forward<Args>(args)...);
		}) {
			using type = hz::remove_reference_t<F>;
	...
	private:
		...
		R specific_invoke(Args... args) {
			return (*std::launder(reinterpret_cast<hz::remove_reference_t<F>*>(&storage)))(std::forward<Args>(args)...);
		}

		template<typename F>
		void specific_destroy() {
			using type = hz::remove_reference_t<F>;
			std::launder(reinterpret_cast<type*>(&storage))->~type();
		}

		template<typename F>
		void specific_move(small_function& other) {
			using type = hz::remove_reference_t<F>;
			new (other.storage) type {std::move(*std::launder(reinterpret_cast<type*>(&storage)))};
			other.invoke = invoke;
